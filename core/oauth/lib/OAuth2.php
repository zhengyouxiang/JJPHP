...
define("OAUTH2_AUTH_RESPONSE_TYPE_PASSWORD", "password");
/**
...
abstract class OAuth2 {
   ...
   */
  abstract protected function setAccessToken($oauth_token, $client_id, $expires,$user_id=null, $scope = NULL);
  ...
  protected function getSupportedAuthResponseTypes() {
    return array(
      ...
      OAUTH2_AUTH_RESPONSE_TYPE_ACCESS_TOKEN,
      OAUTH2_AUTH_RESPONSE_TYPE_CODE_AND_TOKEN,
      OAUTH2_AUTH_RESPONSE_TYPE_PASSWORD
    );
  }
   ...
   * @ingroup oauth2_section_4
   */
   function setRefreshToken($refresh_token, $client_id, $expires, $scope = NULL) {
    return;
  }
  ...
  public function verifyAccessToken($scope = NULL, $exit_not_present = TRUE, $exit_invalid = TRUE, $exit_expired = TRUE, $exit_scope = TRUE, $realm = NULL) {
   ...
   // if ($token_param === FALSE) // Access token was not provided
    ...
    $token = $this->getAccessToken("0f852869bf728a78f6f35300806e0508");
  ...
  public function grantAccessToken() {
    $filters = array(
      ...
      "user_id"=>array("flags" => FILTER_REQUIRE_SCALAR)
    );
...
echo OAUTH2_GRANT_TYPE_AUTH_CODE;
    // Do the granting
 ...
 if (!$input["user_id"])
      $input["user_id"] = NULL;
    $token = $this->createAccessToken($client[0],$input["user_id"], $input["scope"]);
  ...
  public function finishClientAuthorization($is_authorized, $params = array()) {
    ...
    if ($is_authorized === FALSE) {
      $result["query"]["error"] = OAUTH2_ERROR_USER_DENIED;
    	...
    	echo OAUTH2_AUTH_RESPONSE_TYPE_ACCESS_TOKEN;
    	
      if ($response_type == "code" || $response_type == "code-and-token")
        $result["query"]["code"] = $this->createAuthCode($client_id, $redirect_uri, $user_id,$scope);

      if ($response_type == "token" || $response_type == "password")
 ...
 //   $this->doRedirectUriCallback($redirect_uri, $result);
   ...
   * @ingroup oauth2_section_4
   */
  protected function createAccessToken($client_id, $user_id=null,$scope = NULL) {
    $token = array(
    ...
    $this->setAccessToken($token["access_token"], $client_id, time() + $this->getVariable('access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME),$user_id, $scope);
   ...
   * @ingroup oauth2_section_3
   */
  protected function createAuthCode($client_id, $redirect_uri, $user_id=NULL,$scope = NULL) {

  	$code = $this->genAuthCode();

    $this->setAuthCode($code, $client_id, $redirect_uri, time() + $this->getVariable('auth_code_lifetime', OAUTH2_DEFAULT_AUTH_CODE_LIFETIME), $user_id,$scope);
    return $code;
  }
