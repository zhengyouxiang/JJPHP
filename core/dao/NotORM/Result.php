...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable {
	...
	protected $data, $referencing = array(),$realdata=array(), $aggregation = array(), $accessed, $access, $keys = array();
	...
	*/
	 ...
	 * @param $jontype
	 */
	function join($table,$on=null,$jontype="INNER")
	{
		$temp="";
		if($on ==null||!isset($on))
		{
			$temp=" ".$jontype." JOIN  ".$table;
		}
		else 
		{
		  $temp=" ".$jontype." JOIN  ".$table." ON ".$on;
		}
		$this->joinon[]=$temp;
		return $this;
	}
	...
	*/
	function __toString() {
		...
		$return .= " FROM $this->table " .$joinon." " . $this->whereString();
		if ($this->union) {
	...
	protected function query($query, $parameters) {
		...
		else if($this->type=='mysql'||$this->type=='mysqli')
		{
			foreach ($parameters as $temp)
            {
            	$temp=$this->quote($temp);
             $query=preg_replace('/\?(.*)/',$temp.'${1}',$query);
            }
           

		 return		 $this->notORM->JJDAO->_execute($query);
		}
	}
	...
	*/
	function insert($data) {
		...
		if (!is_array($data)) {
			...
			elseif ($this->type=='mysql'||$this->type=='mysqli')
			{
				if($this->notORM->JJDAO->lastInsertId())
				{
			return $this->notORM->JJDAO->lastInsertId();
				}
				else 
				{
		...
		if (!isset($data[$this->primary]) && ($id = $this->notORM->JJDAO->lastInsertId())) {
	...
	*/
	function aggregation($function) {
		...
		elseif($this->type=='mysql'||$this->type=='mysqli')
		{
			$result=$this->query($query, $this->parameters);
			 $result=$this->notORM->JJDAO->fetch_array($result);
			 return $result[0];
		}
	...
	protected function execute() {
		if (!isset($this->rows)) {
			...
			else if($this->type=='mysql'||$this->type=='mysqli')
			{
				if ($result) {
					$key=0;
		while(($row =$this->notORM->JJDAO->fetch_assoc($result)) != false) {
		...
		$this->notORM->JJDAO->_free_result();
				}
			}
			$this->data = $this->rows;
		}
   ...
   function fetchOneData()
	{
		$this->execute();
		if (count($this->realdata)>0)
		{
		return $this->realdata[0];
		}
		else
		{
			return NULL;
	...
	function fetchObj()
	{
	   if (!isset($this->rows)) {
			...
			if ($result) {
				...
				{
					$retrunarr=array();
					while ($row=$this->notORM->JJDAO->fetch_object($result))
					{
						$retrunarr[]=$row;
					}
				    $this->notORM->JJDAO->free_result($result);
					return $retrunarr;
				}
